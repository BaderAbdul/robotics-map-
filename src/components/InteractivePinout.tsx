...
const pins = [
  ...
  { id: 'ioref', name: 'IOREF', group: 'top', type: 'power', icon: Zap, color: 'text-orange-400', glow: 'shadow-[0_0_15px_#f97316]', desc: 'مرجع الجهد للمتحكم (IOREF).' },
  ...
  { id: '3v3', name: '3.3V', group: 'top', type: 'power', icon: Zap, color: 'text-orange-400', glow: 'shadow-[0_0_15px_#f97316]', desc: 'مخرج طاقة 3.3 فولت.' },
  ...
  { id: 'gnd1', name: 'GND', group: 'top', type: 'gnd', icon: PowerOff, color: 'text-slate-400', glow: 'shadow-[0_0_15px_#94a3b8]', desc: 'القطب السالب (الأرضي).' },
  ...
  { id: 'vin', name: 'Vin', group: 'top', type: 'power', icon: Zap, color: 'text-red-500', glow: 'shadow-[0_0_15px_#ef4444]', desc: 'مدخل طاقة ببطارية خارجية (7-12 فولت).' },
  ...
  { id: 'aref', name: 'AREF', group: 'top', type: 'analog', icon: Activity, color: 'text-green-600', glow: 'shadow-[0_0_15px_#166534]', desc: 'الجهد المرجعي للمداخل التناظرية.' },
  { id: 'sda', name: 'SDA', group: 'top', type: 'i2c', icon: Wifi, color: 'text-teal-400', glow: 'shadow-[0_0_15px_#2dd4bf]', desc: 'قطب بيانات I2C (SDA).' },
  { id: 'scl', name: 'SCL', group: 'top', type: 'i2c', icon: Wifi, color: 'text-teal-400', glow: 'shadow-[0_0_15px_#2dd4bf]', desc: 'قطب ساعة I2C (SCL).' },
  { id: 'd13', name: '13', group: 'top', type: 'digital', icon: Hash, color: 'text-blue-400', glow: 'shadow-[0_0_15px_#60a5fa]', desc: 'مخرج/مدخل رقمي (يحتوي على LED مدمج).' },
  { id: 'd12', name: '12', group: 'top', type: 'digital', icon: Hash, color: 'text-blue-400', glow: 'shadow-[0_0_15px_#60a5fa]', desc: 'مخرج/مدخل رقمي.' },
  { id: 'd11', name: '~11', group: 'top', type: 'pwm', icon: Activity, color: 'text-purple-400', glow: 'shadow-[0_0_15px_#a855f7]', desc: 'مخرج PWM (نبضات).' },
  ...
  // --- صف الأقطاب السفلي الأيمن (8 أقطاب Digital/PWM/Serial) ---
  ...
  { id: 'tx', name: 'TX1', group: 'bottom_right', type: 'serial', icon: Wifi, color: 'text-green-500', glow: 'shadow-[0_0_15px_#16a34a]', desc: 'إرسال تسلسلي (TX).' },
  { id: 'rx', name: 'RX0', group: 'bottom_right', type: 'serial', icon: Wifi, color: 'text-green-500', glow: 'shadow-[0_0_15px_#16a34a]', desc: 'استقبال تسلسلي (RX).' },

  // --- صف الأقطاب السفلي الأيسر (6 أقطاب Analog In) ---
  { id: 'a0', name: 'A0', group: 'bottom_left', type: 'analog', icon: Activity, color: 'text-teal-400', glow: 'shadow-[0_0_15px_#2dd4bf]', desc: 'مدخل تناظري.' },
  ...
  { id: 'a4', name: 'A4', group: 'bottom_left', type: 'analog', icon: Activity, color: 'text-teal-400', glow: 'shadow-[0_0_15px_#2dd4bf]', desc: 'مدخل تناظري.' },
  { id: 'a5', name: 'A5', group: 'bottom_left', type: 'analog', icon: Activity, color: 'text-teal-400', glow: 'shadow-[0_0_15px_#2dd4bf]', desc: 'مدخل تناظري.' },
];
...
export default function InteractivePinout() {
  ...
  const PinSocket = ({ pin, labelSide = 'top' }: { pin: any, labelSide?: 'top' | 'bottom' }) => {
    ...
    return (
      ...
      >
        {/* المسمار (تم تصغيره وإزالة الحواف ليكون متلاصقاً) */}
        ...
        {/* اسم القطب */}
        <span className={`absolute ${labelSide === 'top' ? '-top-4' : '-bottom-4'} text-[7px] font-mono font-bold transition-colors whitespace-nowrap ${isHovered ? pin.color : 'text-teal-500/60'}`}>
          {pin.name}
        </span>
      </div>
    );
  ...
  return (
    <div className="flex flex-col items-center gap-8 py-4 overflow-hidden">
      <p className="text-slate-400 text-xs text-center leading-relaxed">
        الآن هذا هو الأردوينو الحقيقي! مرر الماوس فوق المنافذ لمعرفة وظيفتها.
      </p>
      
      {/* حاوية 3D مع Scale للحماية من الشاشات الصغيرة */}
      <div className="relative w-full h-[280px] flex items-center justify-center scale-[0.65] sm:scale-90 md:scale-100 origin-center" style={{ perspective: '1200px' }}>
        
        {/* اللوحة الخضراء (تثبيت العرض min-w-[450px] لمنع تشوه التصميم) */}
        <div 
          className="relative w-[450px] min-w-[450px] h-[240px] bg-[#1e5a60] rounded-xl shadow-[-20px_20px_40px_rgba(0,0,0,0.6)] border-2 border-[#164246] p-4 transition-transform duration-500 hover:rotate-x-[50deg] hover:rotate-z-[-25deg]"
        ...
        >
          ...
          {/* منفذ USB و Power */}
          <div className="w-16 h-12 bg-slate-300 border border-slate-400 rounded-sm absolute bottom-2 left-[-10px] flex items-center justify-center shadow-lg" style={{ transform: 'translateZ(10px)' }}>
             <div className="w-10 h-6 bg-slate-100 rounded-sm border border-slate-300"></div>
          </div>
          <div className="w-10 h-14 bg-black border-2 border-slate-800 rounded-r-md absolute top-[-5px] left-[-2px] shadow-lg flex items-center justify-center" style={{ transform: 'translateZ(10px)' }}>
             <div className="w-6 h-6 bg-slate-900 rounded-full border border-slate-700"></div>
          </div>

          {/* شريحة ATmega328P */}
          <div className="w-28 h-8 bg-[#0f172a] rounded-sm absolute center flex items-center justify-center shadow-xl" style={{ transform: 'translateZ(5px)' }}>
              <span className="text-[6px] font-mono text-slate-500">ATMEGA328P</span>
          </div>

          {/* --- صف المنافذ العلوي (18 قطب - متلاصقة بـ gap-[1px]) --- */}
          <div className="absolute top-2 right-4 flex flex-col items-end" style={{ transform: 'translateZ(8px)' }}>
          ...
          <div className="absolute bottom-3 right-4 flex flex-col items-end" style={{ transform: 'translateZ(8px)' }}>
            <div className="flex items-center gap-[1px] bg-black p-[2px] rounded-sm border border-white/5 shadow-md">
          ...
          <div className="absolute bottom-3 left-20 flex flex-col items-start" style={{ transform: 'translateZ(8px)' }}>
            <div className="flex items-center gap-[1px] bg-black p-[2px] rounded-sm border border-white/5 shadow-md">
      ...
      {/* بطاقة الشرح الداكنة */}
      <div className="h-32 w-full bg-[#0b1120] border border-slate-800 rounded-2xl p-5 relative overflow-hidden flex items-center shadow-xl mt-4">
          ...
          {activePinData ? (
            ...
            >
             ...
             <motion.p key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-xs text-slate-500 font-medium text-center w-full">
                قم بتحديد أحد المنافذ على اللوحة لعرض الشرح الدقيق.
             </motion.p>
          )}
        </AnimatePresence>
      </div>
